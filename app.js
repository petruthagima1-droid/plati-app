...
function renderBills() {
  ...
  const groupsHtml = groups
    ...
    const billsHtml = unpaidBills.map((bill, bi) => {
      ...
      const delay = Math.min(rowCounter * 35, 420);
      rowCounter++;
      return `
        <div class="bill" data-index="${billIndex}" style="animation-delay:${delay}ms">
    ...
    return `
      <div class="group" style="animation-delay:${Math.min(gi * 50, 300)}ms">
...
// Plays a short checkmark-pop + green flash, then collapses the row before
// actually committing the paid state (which triggers the real re-render).
function animateCheckAndToggle(checkboxEl, bill) {
  const row = checkboxEl.closest(".bill");
  checkboxEl.classList.add("checking");
  checkboxEl.textContent = "✓";
  if (row) row.classList.add("row-checking");

  setTimeout(() => {
    if (row) row.classList.add("row-collapsing");
    setTimeout(() => {
      togglePaid(bill);
    }, 320);
  }, 260);
...
// Two-step open/close so display:none can flip before the opacity/transform
// transition starts (a display change can't itself be animated).
function openModalOverlay(id) {
  const overlay = document.getElementById(id);
  overlay.classList.remove("hidden");
  void overlay.offsetWidth; // force reflow so the initial state registers
...
function openEditModal(bill) {
  ...
  openModalOverlay("editModalOverlay");
}

function duplicateModal() {
  if (!editingBill) return;
  const base = editingBill;
  // Modal is already open — just swap its contents in place, no need to close/reopen
  editingBill = null;
...
function closeEditModal() {
  closeModalOverlay("editModalOverlay");
  editingBill = null;
}
