...
function renderBills() {
  ...
  // Next due card: first group with anything unpaid, otherwise the first upcoming group
  const nextGroup = groups.find(g => g.unpaidTotal > 0) || groups[0];
  const nextSettled = nextGroup.unpaidTotal === 0;
  const nextCardHtml = `
    <div class="next-due-card ${nextSettled ? "settled" : ""}">
      <div>
        <div class="label">${nextSettled ? "Plătit integral" : "Următoarea plată"}</div>
        <div class="date">${formatSheetDate(nextGroup.date)}</div>
      </div>
      <div class="amounts">
        <div class="total">${formatLei(nextGroup.total)}</div>
        ${!nextSettled ? `<div class="remaining">rest: ${formatLei(nextGroup.unpaidTotal)}</div>` : ""}
      </div>
    </div>`;
  ...
  main.innerHTML = nextCardHtml + groupsHtml;
